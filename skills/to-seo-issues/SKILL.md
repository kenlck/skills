...
Inspect existing child issues. Propose adding or removing `ready-for-seo-agent` when hard prerequisites, access, approvals, or review dates have changed. A failed parallel external bet never removes readiness from otherwise executable work.
...
Create outcome-review packages up front. Route code-heavy work to an Engineering handoff; do not invoke `to-issues`. Classify dependencies, keep external uncertainty off the hard path where a valid fallback exists, and withhold readiness from packages that require unstated authority or inputs.
...
**Complete when:** every approved issue exists once, parent and dependency references resolve, labels reflect current readiness, the strategy backlink resolves, and every engineering command contains its real issue URL.
